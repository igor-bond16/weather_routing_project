...
class KCS:
    def __init__(self,is_full_scale=False):
        ...
        self.c3 = 0.56 * param["A_BT"]**1.5 / (param["B_wl"] * param["T"] * (0.31*np.sqrt(param["A_BT"]) + param["T"] - param["h_B"]))
        ...
        # --- 2. 形状影響係数 (1+k1) の計算 (Holtrop-Mennen 1984) ---
        ...
        if self.is_full_scale:
            self.Ca = 0.006 * (param["L_wl"] + 100)**(-0.16) - 0.00205 + 0.003 * np.sqrt(param["L_wl"] / 7.5) * (param["Cb"]**4) * self.c2 * (0.04 - c4)
        else:
            self.Ca = 0.0
    ...
    def calc_calm_water_resistance(self, v_ms):
     #   v_ms = v_knots * 0.51444
       ...
       # c2 = np.exp(-1.89 * np.sqrt(c3))
        ...
        # 造波抵抗本体
        Rw = (
            c1 * self.c2 * c5 * param["nabla"] * param["rho_water"] * param["g"] * 
